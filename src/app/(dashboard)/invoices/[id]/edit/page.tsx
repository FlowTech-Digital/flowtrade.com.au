...
type LineItem = {
  id: string
  db_id?: string // Database ID for existing items
  description: string
  quantity: number
  unit_price: number
  amount: number
...
function calculatePaymentTerms(issueDate: string, dueDate: string): number {
  if (!issueDate || !dueDate) return 14
  const issue = new Date(issueDate)
  const due = new Date(dueDate)
  const diffTime = due.getTime() - issue.getTime()
  const diffDays = Math.round(diffTime / (1000 * 60 * 60 * 24))
  // Return closest standard term
...
export default function EditInvoicePage({ params }: { params: Promise<{ id: string }> }) {
    ...
    async function loadInvoice() {
      ...
      const { data: invoice, error: invoiceError } = await supabase
        ...
        .select(`
          *,
          customer:customers(*),
          job:jobs(id, job_number, status, quoted_total)
        `)
      ...
      // Calculate payment terms from dates
      const terms = calculatePaymentTerms(invoice.issue_date, invoice.due_date)

      setFormData({
        customer_id: invoice.customer_id,
        customer: invoice.customer as Customer,
        job_id: invoice.job_id,
        job: invoice.job as Job | null,
      ...
      if (!lineItems || lineItems.length === 0) {
        setFormData(prev => ({
          ...prev,
          line_items: [{ id: generateLineItemId(), description: '', quantity: 1, unit_price: 0, amount: 0, isNew: true }]
        }))
      }
  ...
  }, [customerSearch, orgId, showCustomerSearch])
  ...
  const addLineItem = () => {
    setFormData(prev => ({
      ...
      line_items: [
        ...prev.line_items,
        { id: generateLineItemId(), description: '', quantity: 1, unit_price: 0, amount: 0, isNew: true }
      ]
    }))
  }
  ...
  const removeLineItem = (id: string) => {
    setFormData(prev => {
      const item = prev.line_items.find(i => i.id === id)
      if (!item) return prev

      // If it's a new item, just remove it
      if (item.isNew) {
        const remaining = prev.line_items.filter(i => i.id !== id)
        // Ensure at least one line item
        if (remaining.filter(i => !i.isDeleted).length === 0) {
          return {
            ...prev,
            line_items: [{ id: generateLineItemId(), description: '', quantity: 1, unit_price: 0, amount: 0, isNew: true }]
          }
        }
        return { ...prev, line_items: remaining }
      }

      // If it's an existing item, mark as deleted
      const updated = prev.line_items.map(i => 
        i.id === id ? { ...i, isDeleted: true } : i
      )
      
      // If all non-deleted items are gone, add a new empty one
      if (updated.filter(i => !i.isDeleted).length === 0) {
        return {
          ...prev,
          line_items: [...updated, { id: generateLineItemId(), description: '', quantity: 1, unit_price: 0, amount: 0, isNew: true }]
        }
      }
  ...
  const activeLineItems = formData.line_items.filter(item => !item.isDeleted)
  ...
  const isValid = () => {
    return formData.customer_id && 
           activeLineItems.some(item => item.description && item.amount > 0)
  }
  ...
  const handleSave = async () => {
    ...
    const deletedItems = formData.line_items.filter(item => item.isDeleted && item.db_id)
    ...
    const existingItems = formData.line_items.filter(item => !item.isDeleted && !item.isNew && item.db_id)
    ...
    // 3. Insert new items
    const newItems = formData.line_items.filter(item => !item.isDeleted && item.isNew && item.description && item.amount > 0)
    if (newItems.length > 0) {
      const maxSortOrder = Math.max(...existingItems.map((_, i) => i), -1)
      const lineItemsToInsert = newItems.map((item, index) => ({
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Edit Invoice {invoiceNumber}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-flowtrade-navy-light rounded-xl border border-flowtrade-navy-lighter p-6">
            ...
            {formData.customer && !showCustomerSearch ? (
              <div className="bg-flowtrade-navy rounded-lg border border-flowtrade-cyan/30 p-4">
                <div className="flex items-start justify-between">
                  ...
                  <button
                    ...
                    className="text-flowtrade-cyan hover:text-flowtrade-cyan/80 transition-colors text-sm"
                  >
                    Change
                  </button>
                </div>
          ...
          <div className="bg-flowtrade-navy-light rounded-xl border border-flowtrade-navy-lighter p-6">
            ...
            <div className="space-y-3">
              {formData.line_items.filter(item => !item.isDeleted).map((item, index) => (
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-medium capitalize ${
                  invoiceStatus === 'paid' ? 'text-green-400' :
                  invoiceStatus === 'sent' ? 'text-blue-400' :
                  invoiceStatus === 'overdue' ? 'text-red-400' :
                  'text-gray-400'
                }`}>{invoiceStatus}</span>
              </div>
